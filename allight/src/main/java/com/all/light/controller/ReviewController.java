...
public class ReviewController {
	...
	public ModelAndView reviewUpdateGet(
		...
		String id = (String)request.getSession().getAttribute("MID");
		//비즈니스 로직
		revDTO = revSVC.getReviewInfo(num, id);
		//모델지정
	...
	public ModelAndView reviewDelete(
			...
			@RequestParam(value = "nowPage", required=false, defaultValue="1") int nowPage,
			ReviewDTO revDTO, ModelAndView mv, RedirectView rv, HttpServletRequest request) {
		System.out.println("\nReviewController.review.delete, num="+num);
		revDTO.setNum(num);
	...
	public ModelAndView itemListCorp(
			...
			ModelAndView mv, RedirectView rv,	HttpServletRequest request) {
		// 파라미터 받기
		String id = (String) request.getSession().getAttribute("COID");
		System.out.println("\nReviewController.itemListCorp, 접속COID = " + id);

		// 페이지 객체에 검색어와 현재 페이지를 넘기고 공지 리스트를 반환
		PageUtil pInfo = revSVC.getPageInfoCorp(id, nowPage, searchWord);
		pInfo.setCoid(id);
	...
	public ModelAndView reviewDeleteCorp(
			...
			@RequestParam(value = "nowPage", required=false, defaultValue="1") int nowPage,
			ReviewDTO revDTO, ModelAndView mv, RedirectView rv, HttpServletRequest request) {
		System.out.println("\nReviewController.reviewdeleteCorp, num="+num);
		revDTO.setNum(num);
		revDTO.setCoid( (String)request.getSession().getAttribute("COID"));
		System.out.println("전달 파라미터 = "+revDTO);
