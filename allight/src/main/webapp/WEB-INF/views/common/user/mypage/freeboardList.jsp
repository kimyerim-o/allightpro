...
<body>
	...
	<form id="sFrm" method="get"
		action="<%=request.getContextPath()%>/mypage/freeboard/list.com">
		<div class="searchDiv">
		<select name="ftype" class="selectCss">
			...
			<a href="<%=request.getContextPath()%>/mypage/freeboard/list.com"><input type="button" value="초기화"/></a>
		</div>
		</form>
		<table class="table">
			<tr>
		...
		<div class="right">
			<a href="<%=request.getContextPath() %>/mypage/freeboard/write.com" class="btn">글쓰기</a>
		</div>
		...
		<div class="center">
			<ul class="pagination">
				...
				<c:forEach begin="${PINFO.startPage}" end="${PINFO.endPage}" var="i">
					<li id="li">
						<!-- 스크립트 적용해야 할것같아요 --> <a
						href="${pageContext.request.contextPath}/mypage/freeboard/list.com?ftype=${param.ftype }&type=${param.type}&search=${param.search}&nowPage=${i}">${i}</a>
					</li>
				</c:forEach>
