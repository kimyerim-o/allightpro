...
Highcharts.chart('container1', {
	  ...
	  xAxis: {
	    categories: [<c:forEach var="list" items="${LIST}" varStatus="status">${list.day},</c:forEach>]
	  },
	  yAxis: {
	    title: {
	      text: 'Weight (Kg)'
	  ...
	  series: [{
		name: '몸무게',
	    data: [<c:forEach var="list" items="${LIST}" varStatus="status"><c:if test="${list.dweight != 0}">${list.dweight},</c:if></c:forEach>]
	  }]
	});
...
Highcharts.chart('container2', {
	  ...
	  series: [{
		name: '음식 칼로리',
	    data: [<c:forEach var="list" items="${LIST}" varStatus="status"><c:if test="${list.dfoodcal != 0}">${list.dfoodcal},</c:if></c:forEach>]
	  }]
	});
...
Highcharts.chart('container3', {
	  ...
	  series: [{
	    name: '운동 칼로리',
	    data: [<c:forEach var="list" items="${LIST}" varStatus="status"><c:if test="${list.dexercal != 0}">${list.dexercal},</c:if></c:forEach>]
	  }]
	});
</script>
</body>
